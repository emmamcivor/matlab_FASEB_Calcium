...
d_serca=30;
y_plot=round(length(y)/2)+round(56/dy);
...
d_serca=1;
y_plot=round(length(y)/2)+round(140/dy);
...
figure(13)
pcolor(XXj,ZZj,squeeze(cj(:,round(length(y)/2),:))')
shading interp
colormap(jet)
colorbar
c=colorbar;
...
figure(15)
pcolor(XXs,ZZs,squeeze(cs(:,y_plot,:))')
shading interp
colormap(jet)
colorbar
c=colorbar;
